...
EXTERNC int *alloc_record(int size, struct descriptor *des_ptr) {
  // 忘记还要修改runtime.cc 中的alloc了了，找了半天！
  uint64_t *sp; // sp为alloc_record的RBP
  GET_RBP(sp);
  sp += 2;
  int i;
...
EXTERNC int string_equal(struct descriptor *s, struct descriptor *t) {
  int i;
  if (s == t)
    return 1;
...
EXTERNC void print(struct descriptor *s) {
  int i;
...
struct descriptor consts[256];
struct descriptor empty = {0, ""};
...
EXTERNC int ord(struct descriptor *s) {
  if (s->length == 0)
    return -1;
  else
    return s->chars[0];
...
EXTERNC struct descriptor *chr(int i) {
  if (i < 0 || i >= 256) {
...
EXTERNC int size(struct descriptor *s) { return s->length; }

EXTERNC struct descriptor *substring(struct descriptor *s, int first, int n) {
  ...
  {
    struct descriptor *t = (struct descriptor *)malloc(sizeof(int) + n);
    int i;
    t->length = n;
    for (i = 0; i < n; i++)
...
EXTERNC struct descriptor *concat(struct descriptor *a, struct descriptor *b) {
  if (a->length == 0)
    return b;
  else if (b->length == 0)
    return a;
...
EXTERNC struct descriptor *__wrap_getchar() {
  int i = getc(stdin);
  if (i == EOF)
    return &empty;
  else
